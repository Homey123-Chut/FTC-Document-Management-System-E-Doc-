...
/// Alternate mock repository for search/index screens.
...
class DocumentMockRepository implements DocumentRepository {
  ...
  Future<Map<String, List<DocumentModel>>> _loadAll() async {
    ...
    final decodedMap = decoded as Map; // promote for use in loop
  ...
  Future<List<DocumentModel>> searchDocuments({
    ...
    final documents = await getDocumentsByType(type);
    ...
    return documents.where((doc) {
      ...
      if (title.isNotEmpty) {
        ...
        final matchesKhmer =
            doc.titleKhmer.toLowerCase().contains(titleLower);
        final matchesLatin =
            doc.titleLatin.toLowerCase().contains(titleLower);
        if (!matchesKhmer && !matchesLatin) {
          return false;
        }
      }
  ...
  Future<void> updateDocument(DocumentModel document) async {
    ...
    for (final entry in all.entries) {
      final index = entry.value.indexWhere((d) => d.id == document.id);
      if (index != -1) {
