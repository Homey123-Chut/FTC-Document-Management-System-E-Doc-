...
import 'package:e_doc_redo/ui/features/document/services/incoming_document_service.dart';
...
class IncomingDocumentController extends GetxController {
  ...
  Future<bool> approveCurrentStep() async {
    ...
    try {
      final updated = await service.approveStep(doc.id.toString());
  ...
  Future<bool> rejectCurrentStep({String? comment}) async {
    ...
    isActionLoading.value = true;
    try {
      final updated = await service.rejectStep(doc.id.toString(), comment: comment);
      document.value = updated;
