...
class SendDocumentController extends GetxController {
  ...
  void _loadCreatorName() {
    try {
      final auth = Get.find<AuthService>();
      creatorName.value = auth.currentUser?.username ?? 'អ្នកបង្កើត';
    } catch (_) {
      creatorName.value = 'អ្នកបង្កើត';
    }
  }
  ...
  /// Submits the outgoing document and navigates back.
  /// On success, inserts into the outgoing DocumentController list,
  /// closes the dialog, and shows a success snackbar.
  Future<void> submitAndNavigate() async {
    final result = await submit();
    if (result != null) {
      try {
        final docCtrl = Get.find<DocumentController>(tag: 'doc_outgoing');
